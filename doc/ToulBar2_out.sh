...
# Usage: cd web ; ../doc/ToulBar2_out.sh
...
toulbar2 EXAMPLES/example.wcsp.xz | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_binary_WCSP_example_s_default.txt
...
toulbar2 EXAMPLES/example.wcsp.xz -i | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_s_WCSP_example_INCOP.txt

#------------
#title "Solve a WCSP with an initial upper bound and save its (first) optimal solution in filename ''example.sol'':"
toulbar2 EXAMPLES/example.wcsp.xz -ub=28 -w=example.sol | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_s_WCSP_example_initial_upper_bound.txt

#title "commande cat example.sol ..."
cat example.sol > ../doc/out_save_examplesol.txt
...
toulbar2 EXAMPLES/scen06.wcsp.xz -l -vacint | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_larger_WCSP_scen06_s_VAC.txt
...
toulbar2 EXAMPLES/scen06.wcsp.xz EXAMPLES/scen06.dec -vns -time=10 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_cluster_decomp_scen06dec_s_UDGVNS.txt
...
toulbar2 EXAMPLES/scen07.wcsp.xz -vns -O=-1 -E -time=5 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_another_instance_scen07_s_UDGVNS.txt
...
toulbar2 EXAMPLES/404.wcsp.xz -O=-3 -B=1 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_404_s_BTD_HBFS.txt

#------------
#title "Solve the same problem using Russian Doll Search exploiting BTD~\\cite{Sanchez09a}:"
toulbar2 EXAMPLES/404.wcsp.xz -O=-3 -B=2 | awk '/^Read /{ok=1} /Solving cluster subtree 44 /{ok=1} ok{print $0} /Solving cluster subtree 7 /{ok=0;print "";print "...";print ""}' > ../doc/out_s_same_404_russian_doll_search.txt
...
toulbar2 EXAMPLES/505.wcsp.xz -B=3 -j=1 -svo -k=1 | awk '/^Read /{ok=1} /Solving cluster subtree 3 /{ok=1} ok{print $0} /Solving cluster subtree 2 /{ok=0;print "";print "...";print ""}' > ../doc/out_s_another_WCSP_505_russian_doll_search.txt
...
mpirun -n 4 toulbar2 EXAMPLES/505.wcsp.xz -vns -O=-3 -time=5 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_s_same_WCSP_505_UPDGVNS_minfill_cluster_decomp.txt
...
toulbar2 EXAMPLES/example.wcsp.xz EXAMPLES/example.dec -vns | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_cluster_decomp_s_UDGVNS_exampledec.txt

#------------
#title "Solve a WCSP using a parallel variable neighborhood search algorithm (UPDGVNS) with the same cluster decomposition:"
mpirun -n 4 toulbar2 EXAMPLES/example.wcsp.xz EXAMPLES/example.dec -vns | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_s_WCSP_parallel_UPDGVNS_same_exampledec.txt
...
toulbar2 EXAMPLES/example.wcsp.xz EXAMPLES/example.order -B=1 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_exampleorder_s_BTD_HBFS.txt
...
toulbar2 EXAMPLES/example.wcsp.xz EXAMPLES/example.cov -B=1 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_examplecov_s_BTD_HBFS_tree_decomp.txt
...
toulbar2 EXAMPLES/pedigree9.uai.xz -O=-3 -p=-8 -B=1 -r=4 | grep -v "Optimality gap:" | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_MRF_pedigree9_UAI_format_s.txt
...
toulbar2 EXAMPLES/GeomSurf-7-gm256.uai.xz -A -V -vacint | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_another_MRF_GeomSurf_7_gm256_s.txt
...
toulbar2 EXAMPLES/1CM1.uai.xz -A=1000 -vacint -rasps -vacthr | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_another_MRF_1CM1_s.txt
...
toulbar2 EXAMPLES/brock200_4.clq.wcnf.xz -m=1 | grep -v "Optimality gap:" | awk '/c Read /{ok=1} ok{print $0}' > ../doc/out_d_weighted_MaxSAT_brock200_4_wcnf_format_s.txt

#------------
#title "Download another WCSP file {\\em latin4.wcsp.xz}. Count the number of feasible solutions:"
toulbar2 EXAMPLES/latin4.wcsp.xz -a | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_another_WCSP_latin4_cns.txt
...
toulbar2 EXAMPLES/latin4.wcsp.xz -a=20 -div=12 | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_f_greedy_sequence_latin4.txt
...
toulbar2 EXAMPLES/GEOM40_6.wcsp.xz -O=-3 -a -B=1 -ub=1 -hbfs: | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_d_crisp_CSP_GEOM40_6_cns.txt

#------------
#title "Get a quick approximation of the number of solutions of a CSP with Approx\#BTD~\\cite{Favier09a}:"
toulbar2 EXAMPLES/GEOM40_6.wcsp.xz -O=-3 -a -B=1 -D -ub=1 -hbfs: | awk '/^Read /{ok=1} ok{print $0}' > ../doc/out_g_quick_approximation_GEOM40_6_cns.txt
